...
public class DocumentStoreImpl implements DocumentStore {
    ...
    public int putDocument(InputStream input, URI uri, CompressionFormat format) {
        if (uri == null) {
            return Integer.MIN_VALUE; //If there was an error reading the file, what should I do here?
        }
        if (input == null) {
            return this.deleteDocument(uri) ? Integer.MIN_VALUE : 1;
        }
        try {
    ...
    public boolean deleteDocument(URI uri) {
        ...
        pushDelCommand(uri, hTable.get(uri));
        hTable.put(uri, null);
        return true;
    }
    ...
    private void pushPutCommand(URI uri, DocumentImpl doc) {
        ...
        Function<URI, Boolean> putUndo = (u) -> this.hTable.put(u, null) != null;
        Function<URI, Boolean> putRedo = (u) -> this.hTable.put(u, doc) == null;
    ...
    private void pushOverwriteCommand(URI uri, DocumentImpl newDoc) {
        ...
        Function<URI, Boolean> putUndo = (u) -> this.hTable.put(u, oldDoc) != null;
        Function<URI, Boolean> putRedo = (u) -> this.hTable.put(u, newDoc) != null;
        Command toPush = new Command(uri, putUndo, putRedo);
        commandStack.push(toPush);
    }

    private void pushDelCommand(URI uri, DocumentImpl doc) {
        Function<URI, Boolean> delUndo = (u) -> hTable.put(u, doc) == null;
        Function<URI, Boolean> delRedo = (u) -> this.hTable.put(u, null) != null;
